...
class WaveController:
    ...
    def step(self, iteration, time, timestep, pos=None):
        ...
        for i in range(15):
            ...
            muscles[2*i]=0.5+A*np.sin(2*np.pi*(f*self.times[iteration]-epsilon*i/15))/2
            self.muscle_l[i]=muscles[2*i]
            muscles[2*i+1]=0.5-A*np.sin(2*np.pi*(f*self.times[iteration]-epsilon*i/15))/2
            self.muscle_l[i]=muscles[2*i+1]
        return muscles
